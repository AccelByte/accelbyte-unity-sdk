 ...
 static class SimpleJson
    {
        ...
        public static object DeserializeObject(string json, Type type, IJsonSerializerStrategy jsonSerializerStrategy)
        {
            ...
            return type == null || jsonObject != null && ReflectionUtils.IsAssignableFrom(jsonObject.GetType(), type)
                       ? jsonObject
                       : (jsonSerializerStrategy ?? SimpleJson.CurrentJsonSerializerStrategy).DeserializeObject(jsonObject, type);
        }
        ...
        static int NextToken(char[] json, ref int index)
        {
            ...
            {
                case '{':
                    return SimpleJson.TOKEN_CURLY_OPEN;
                case '}':
                    return SimpleJson.TOKEN_CURLY_CLOSE;
                case '[':
                    return SimpleJson.TOKEN_SQUARED_OPEN;
                case ']':
                    return SimpleJson.TOKEN_SQUARED_CLOSE;
                case ',':
                    return SimpleJson.TOKEN_COMMA;
                case '"':
                    return SimpleJson.TOKEN_STRING;
                case '0':
                case '1':
                case '2':
                case '3':
                ...
                case '9':
                case '-':
                    return SimpleJson.TOKEN_NUMBER;
                case ':':
                    return SimpleJson.TOKEN_COLON;
            }
            index--;
            int remainingLength = json.Length - index;
            // false
        ...
        static bool SerializeObject(IJsonSerializerStrategy jsonSerializerStrategy, IEnumerable keys, IEnumerable values, StringBuilder builder)
        {
            ...
            while (ke.MoveNext() && ve.MoveNext())
            {
                ...
                if (stringKey != null)
                    ...
                    if (!SimpleJson.SerializeValue(jsonSerializerStrategy, value, builder)) return false;
                builder.Append(":");
        ...
        public static IJsonSerializerStrategy CurrentJsonSerializerStrategy
        {
            get
            {
                return SimpleJson._currentJsonSerializerStrategy ??
                    (SimpleJson._currentJsonSerializerStrategy =
 ...
 class PocoJsonSerializerStrategy : IJsonSerializerStrategy
    {
        ...
        {
            this.ConstructorCache = new ReflectionUtils.ThreadSafeDictionary<Type, ReflectionUtils.ConstructorDelegate>(ContructorDelegateFactory);
            this.GetCache = new ReflectionUtils.ThreadSafeDictionary<Type, IDictionary<string, ReflectionUtils.GetDelegate>>(GetterValueFactory);
            this.SetCache = new ReflectionUtils.ThreadSafeDictionary<Type, IDictionary<string, KeyValuePair<Type, ReflectionUtils.SetDelegate>>>(SetterValueFactory);
        }
 ...
 class DataContractJsonSerializerStrategy : PocoJsonSerializerStrategy
    {
        ...
        internal override IDictionary<string, ReflectionUtils.GetDelegate> GetterValueFactory(Type type)
        {
            ...
            foreach (PropertyInfo propertyInfo in ReflectionUtils.GetProperties(type))
            {
                ...
                {
                    MethodInfo getMethod = ReflectionUtils.GetGetterMethodInfo(propertyInfo);
                    if (!getMethod.IsStatic && DataContractJsonSerializerStrategy.CanAdd(propertyInfo, out jsonKey))
                        result[jsonKey] = ReflectionUtils.GetGetMethod(propertyInfo);
                }
            }
            foreach (FieldInfo fieldInfo in ReflectionUtils.GetFields(type))
            {
                if (!fieldInfo.IsStatic && DataContractJsonSerializerStrategy.CanAdd(fieldInfo, out jsonKey))
        ...
        internal override IDictionary<string, KeyValuePair<Type, ReflectionUtils.SetDelegate>> SetterValueFactory(Type type)
        {
            ...
            foreach (PropertyInfo propertyInfo in ReflectionUtils.GetProperties(type))
            {
                ...
                {
                    MethodInfo setMethod = ReflectionUtils.GetSetterMethodInfo(propertyInfo);
                    if (!setMethod.IsStatic && DataContractJsonSerializerStrategy.CanAdd(propertyInfo, out jsonKey))
            ...
            foreach (FieldInfo fieldInfo in ReflectionUtils.GetFields(type))
            {
                if (!fieldInfo.IsInitOnly && !fieldInfo.IsStatic && DataContractJsonSerializerStrategy.CanAdd(fieldInfo, out jsonKey))
 ...
 class ReflectionUtils
        {
            ...
            public static Type GetGenericListElementType(Type type)
            {
                ...
                {
                    if (ReflectionUtils.IsTypeGeneric(implementedInterface) &&
            ...
            public static SetDelegate GetSetMethodByExpression(FieldInfo fieldInfo)
            {
                ...
                Action<object, object> compiled = Expression.Lambda<Action<object, object>>(
                    ReflectionUtils.Assign(Expression.Field(Expression.Convert(instance, fieldInfo.DeclaringType), fieldInfo), Expression.Convert(value, fieldInfo.FieldType)), instance, value).Compile();
                return delegate(object source, object val) { compiled(source, val); };
            }
