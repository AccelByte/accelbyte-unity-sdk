...
namespace Oculus.Platform
{
  ...
  public sealed class Core {
    ...
    private static string getAppID(string appId = null) {
      string configAppID = Core.GetAppIDFromConfig();
    ...
    public static void Initialize(string appId = null)
    {
      ...
      if (UnityEngine.Application.isEditor && PlatformSettings.UseStandalonePlatform) {
        var platform = new StandalonePlatform();
        Core.IsPlatformInitialized = platform.InitializeInEditor() != null;
      }
      else if (UnityEngine.Application.platform == RuntimePlatform.WindowsEditor ||
               UnityEngine.Application.platform == RuntimePlatform.WindowsPlayer) {
        var platform = new WindowsPlatform();
        Core.IsPlatformInitialized = platform.Initialize(appId);
      }
      else if (UnityEngine.Application.platform == RuntimePlatform.Android) {
        var platform = new AndroidPlatform();
        Core.IsPlatformInitialized = platform.Initialize(appId);
  ...
  public static partial class Matchmaking
  {
    public class CustomQuery
    {
      ...
      public IntPtr ToUnmanaged()
      {
        ...
        {
          customQueryUnmanaged.criterionArrayCount = (uint) this.criteria.Length;
          var temp = new CAPI.ovrMatchmakingCriterion[this.criteria.Length];

          for(int i=0; i< this.criteria.Length; i++)
          {
            temp[i].importance_ = this.criteria[i].importance;
            temp[i].key_ = this.criteria[i].key;

            if(this.criteria[i].parameters != null && this.criteria[i].parameters.Count > 0)
            {
              temp[i].parameterArrayCount = (uint) this.criteria[i].parameters.Count;
              temp[i].parameterArray = CAPI.ArrayOfStructsToIntPtr(CAPI.DictionaryToOVRKeyValuePairs(this.criteria[i].parameters));
            }
            else
            {
        ...
        {
          customQueryUnmanaged.dataArrayCount = (uint) this.data.Count;
          customQueryUnmanaged.dataArray = CAPI.ArrayOfStructsToIntPtr(CAPI.DictionaryToOVRKeyValuePairs(this.data));
        }
        else
        {
