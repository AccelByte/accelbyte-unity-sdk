...
namespace AccelByte.Server
{
    public class ServerSeasonPassApi
    {
        ...
        public IEnumerator GetCurrentUserSeasonProgression(string @namespace, string accessToken, string userId, 
            ResultCallback<UserSeasonInfoWithoutReward> callback)
        {
            Report.GetFunctionLog(this.GetType().Name);
            Assert.IsNotNull(@namespace, "Can't check user progression! Namespace parameter is null!");
            Assert.IsNotNull(accessToken, "Can't check user progression! AccessToken parameter is null!");
            Assert.IsNotNull(userId, "Can't check user progression! UserId parameter is null!");

            var request = HttpRequestBuilder
                .CreateGet(this.baseUrl + "/seasonpass/admin/namespaces/{namespace}/users/{userId}/seasons/current/progression")
                .WithPathParam("namespace", @namespace)
                .WithPathParam("userId", userId)
                .WithBearerAuth(accessToken)
                .WithContentType(MediaType.ApplicationJson)
